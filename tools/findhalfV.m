% function serves to find the half potential given the conductance and the shift of half potential desired
% cond: the conductance, the actual object for example hx.x.PD.ACurrent
% x_inf: string of either "m_inf" or "h_inf"
% ac_shift: number, the amount of half potential shift desired
...
function [halfV, x_inf_f] = findhalfV(cond, x_inf, ac_shift) % maybe add Ca_average back in the future
  ...
  x_inf = func2str(eval(x_inf)); % evaulate the string in x_inf, and then convert function to string
