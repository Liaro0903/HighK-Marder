function [neuron_name, conds, gates, cond_gates, gate_conds] = xsplit(xconds)
  conds = strings(1, length(xconds));
  gates = strings(1, length(xconds));
  cond_gates = strings(1, length(xconds));
  gate_conds = strings(1, length(xconds));
  for i = 1:length(xconds)
    ...
    gate_conds(i) = strcat(gates(i), '_{', strrep(conds(i), 'Current', ''), '}');
  end
end