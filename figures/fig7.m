...
%% Simulation
target_conds = ["KCa", "Kd"];
dhp_m_mags = [8, 8]; % KCa, Kd
dhp_m_slopes = [-10 -10]; % KCa, Kd
dhp_m_offsets = [-44 -44]; % KCa, Kd

gbars_e = [113.04, 6.28, 8.164, 0.1256, 12.56, 118.4408, 0.0314, 1210.784];

hx = HKX(500*1000, -50, 10);
hx.add_pyloric_neuron('ABPD', 'PD', 'liu-ac', gbars_e);
hx.x.PD.add('huang/EkSwitcher'); % changes the EK
hx.x.PD.add('huang/AlphaSensor');
for i = 1:2
  hx.x.PD.(target_conds(i)).add('huang/ACSensor', 'dhp_m_mag', dhp_m_mags(i), 'dhp_m_slope', dhp_m_slopes(i), 'dhp_m_offset', dhp_m_offsets(i));
end
hx.x.PD.CaT.gbar = 7.2;
hx.x.PD.CaS.gbar = 5.6;
E_L = hx.setKPot(-80);
result = hx.x.integrate;
result.PD = trim_result(result.PD, length(hx.x.time), 100); % trim first 100 sec
...
hp_KCa = -28.3;
hp_Kd = -12.3;
plot(tvec, result.PD.(target_conds{1}).ACSensor(:, 1).*-1 + hp_KCa, 'k', 'LineWidth', 2);
plot(tvec, result.PD.(target_conds{1}).ACSensor(:, 2).*-1 + hp_KCa, 'Color', [233 153 62] ./ 255, 'LineWidth', 2);
plot(tvec, result.PD.(target_conds{2}).ACSensor(:, 1).*-1 + hp_Kd, 'k', 'LineWidth', 2);
plot(tvec, result.PD.(target_conds{2}).ACSensor(:, 2).*-1 + hp_Kd, 'Color', [0.8500, 0.3250, 0.0980], 'LineWidth', 2);
xlabel('Time (s)');
ylabel('V_{1/2} (mV)');
ylim([-40 -10]);
...
for tr = 1:length(currentscape_timeranges)
  ...
  plot(currentscape_timebars(tr, :), [3.7 3.7], 'k');
  if (tr == 1)
