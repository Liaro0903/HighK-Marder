...
% compute firing rate (originally 1e3 but it should be 1e4)
metrics.firing_rate = (1e4*n_spikes)/(length(V)/options.sampling_rate);
...
metrics.min_V_bw_burst_mean = mean(all_min_V_bw_burst, 'omitnan');
% metrics.min_V_in_burst_std = nanstd(all_min_V_bw_burst);
metrics.min_V_in_burst_std = std(all_min_V_bw_burst, 'omitnan');
