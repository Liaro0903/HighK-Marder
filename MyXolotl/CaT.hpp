...
public:
    ...
    double m_inf(double V, double Ca) {return 1.0/(1.0 + exp((V+27.1+ac_shift_m)/-7.2));}
    double h_inf(double V, double Ca) {return 1.0/(1.0 + exp((V+32.1+ac_shift_h)/5.5));}
    double tau_m(double V, double Ca) {return 43.4 - 42.6/(1.0 + exp((V+68.1)/-20.5));}
    double tau_h(double V, double Ca) {return 210.0 - 179.6/(1.0 + exp((V+55.0)/-16.9));}
