...
for pot = -80:pot_dV:-70
  ...
  % myplot2
  x2.output_type = 2;
  x2.t_end = 5000; % want to plot halfway of -80 and -70
  ...
  if pot == -80 % get bursting frequency
    [IBI, B_freq] = burstinginterval(results_and_spiketimes.PD.spiketimes);
  ...
  if pot == -70 % integrate with the change of gbars
    ...
    results_and_spiketimes = x2.integrate;
    all_ABPD_V(:, 3) = results_and_spiketimes.AB.V;
    all_ABPD_V(:, 4) = results_and_spiketimes.PD.V;
    [IBI, B_freq] = burstinginterval(results_and_spiketimes.PD.spiketimes);
    B_freq_after = B_freq;
  end
...
x2_title = ['Model ', int2str(model), ' | gmax AB-PD: ', int2str(base * 4), ' | gmax PD-AB: ', int2str(base), ... 
  ' | freq before: ', num2str(B_freq_before), ' | freq after: ', num2str(B_freq_after)];
x2.myplot2(x2_title, all_ABPD_V, {'AB', 'PD', 'AB-A*2', 'PD-A*2'});
